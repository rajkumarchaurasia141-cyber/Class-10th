...
import { CLASS_10_DATABASE, Class10ChapterData } from '../data/class10SubjectData';
...
export const SubjectsExplorer: React.FC<SubjectsExplorerProps> = ({
  ...
  const [testTimeLeft, setTestTimeLeft] = useState(300); // 300s = 5 mins
  ...
  useEffect(() => {
    ...
    } else if (testTimeLeft === 0 && !testSubmitted) {
      setTestSubmitted(true);
    }
    return () => clearInterval(timer);
  }, [isTestMode, testSubmitted, testTimeLeft]);
  ...
  const currentSubject = CLASS_10_DATABASE[selectedSubjectId] || CLASS_10_DATABASE['maths'];
  const chapters = currentSubject.chapters;
  const currentChapter: Class10ChapterData =
    chapters.find((c) => c.chapter_no === selectedChapterNo) || chapters[0];
  ...
  return (
    <div className="space-y-5 pb-20 animate-fade-in">
      ...
      <div className="flex gap-2 overflow-x-auto pb-1 scrollbar-none">
        {Object.values(CLASS_10_DATABASE).map((sub) => {
